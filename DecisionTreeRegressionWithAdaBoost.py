...
clf=AdaBoostRegressor(DecisionTreeRegressor(max_depth=16),
                          n_estimators=300, random_state=2351)
...
data['Decision Tree Regression with AdaBoost']= pd.DataFrame(data =y_predict_lin[-300:],  index=df.index[-300:])
data.to_csv(path_or_buf='Decision Tree Regression with AdaBoost output.csv' , sep=',' )