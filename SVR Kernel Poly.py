...
clf=SVR(kernel='poly', C=1.0, degree=3, epsilon=0.2)
...
plt.title('Support Vector Machine Regression(Kernel=Poly)')
plt.xticks(x-w/2, ('2017-04-20', '2017-04-21','2017-04-22', '2017-04-23','2017-04-24'))
plt.legend()
plt.show()

data = pd.DataFrame(index=df.index[-200:])
data['Support Vector Machine Regression(Kernel=Poly)']= pd.DataFrame(data =y_predict_lin[-200:],  index=df.index[-200:])
data.to_csv(path_or_buf='Support Vector Machine Regression(Kernel=Poly) output.csv' , sep=',' )