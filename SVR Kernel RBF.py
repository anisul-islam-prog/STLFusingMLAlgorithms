...
clf= SVR(kernel='rbf', C=1.0, gamma='auto', epsilon=0.2)
...
plt.title('Support Vector Machine Regression(Kernel=RBF)')
plt.xticks(x-w/2, ('2017-04-20', '2017-04-21','2017-04-22', '2017-04-23','2017-04-24'))
plt.legend()
plt.show()

data = pd.DataFrame(index=df.index[-300:])
data['Support Vector Machine Regression(Kernel=RBF)']= pd.DataFrame(data =y_predict_lin[-300:],  index=df.index[-300:])
data.to_csv(path_or_buf='Support Vector Machine Regression(Kernel=RBF) output.csv' , sep=',' )